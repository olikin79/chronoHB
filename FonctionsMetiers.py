...
dossier_www = os.path.join(os.getcwd(), "www")
...
class DictionnaireDeCoureurs(dict) :
    ...
    def ajouter(self, coureur, course="", dossard = ""): # course serait une lettre "A", "B", "C", ...
        ...
        if self.existe(coureur) == "" :
            ...
            if dossard : # le dossard est spécifié, on impose le numéro en espérant que celui soit correct. On vérifie tout de même s'il n'est pas occupé.
                c = self.recuperer(coureur.dossard)
                if c.nom and c.prenom and c.sexe : # le coureur existe déjà dans la base, on n'ajoute pas celui demandé.
                    ...
                    return Erreur(10, courteDescription="Le dossard imposé dans le fichier importé existe déjà. Erreur de l’utilisateur. Le coureur n’est pas importé.")
                ...
                if dossardDisponible != "0A" :
                    coureur.setDossard(dossardDisponible)
                    if DEBUG :
                        print("Prochain dossard disponible trouvé :", formateDossardNG(dossardDisponible, course=course))
                    if depuisCoureursElimines : # on écrase un coureurVide
                        self[course][int(dossardDisponible[:-1])-1] = coureur
                    else :
                        self[course].append(coureur)
                    self.evolutionDUnAuxEffectifsTotaux(coureur, evolution=1)
    ...
    def prochainDossardDisponible(self, course, coureur):
            ...
            while i < len(self["CoureursElimines"][course]) and pasTrouve :
                if self["CoureursElimines"][course][i] not in self.dossardsPerdus.get(course, []) and \
                    numeroMinimal <= self["CoureursElimines"][course][i] :
                        ...
                        pasTrouve = False 
                        break
                if self["CoureursElimines"][course][i] > numeroMaximal :
                    break # on sort de la boucle si le dossard est supérieur à la plage autorisée. Inutile de continuer à chercher.
                i += 1
            if not pasTrouve : # on a trouvé un dossard correspondant aux critères. Dans le cas contraire, on passe à la suite 
        ...
        if dossardAttribue == "0A" :
            if Parametres["plusieursSeriesDeCouleurSuccessives"] :
                # les séries de couleur sont activées, des coureurs vides ont préalablement été ajoutés par initialiseSeriesDeCouleursSuccessives()
                # si on arrive ici, c'est qu'ils ont tous été réattribués. L'interface devra retourner une erreur liée au fait que cette maéthode retourne "0A".
                print("Ne devrait jamais survenir sauf s'il n'y a plus de dossard disponible dans la plage.")
            else :
...
class Coureur():#persistent.Persistent):
    ...
    def categorieSansSexe(self) :
        try : 
            retour = self.categorie(Parametres["CategorieDAge"])[:2]
        except :
            retour = None
        return retour
            ...
            if  nom_antenne not in Parametres["donneesRFID"].listeNomsAntennesSecondaires :
                ...
                if chaineAAjouterDansFichierPique :
                    with open(os.path.join(dossier_data_txt,"donneesSmartphone-pique-" + nom_antenne + ".txt"), "a") as file:
                        ...
                        file.close()
    if chaineAAjouterDansFichierTexte :
...
def generateImpressionsNG(uniquementCoursesEtChallenge = False) :
    ...
    with open("./modeles/impression-en-teteSGpments.txt", 'r',encoding="utf-8") as f :
        if Parametres["CategorieDAge"] == 2 :
            enteteSGpments = [f.read().replace("@categorie@","établissement")]
        elif Parametres["CategorieDAge"] == 1 :
            enteteSGpments = [f.read().replace("@categorie@","catégorie")]
        else :
            enteteSGpments = [f.read().replace("@categorie@","classe")]
    f.close()
    ...
    liste_fichiers_pdf_complete=glob.glob(pathImpressions+os.sep+"**"+os.sep+'*.pdf',recursive = True)
...
#         enTeteDesStatistiquesParCategories = """\\textbf{Statistiques par @categorie@ :}
    ...
    for classe in ResultatsGroupementsPourImpressions :
        ...
        if (not estChallenge(classe)) :# and len(groupementAPartirDeSonNom(classe, nomStandard = True).listeDesCourses) > 1) : # si c'est un groupement ET qu'il comporte plus d'une catégorie, on génère un fichier dédié.
            ...
            if ArrDispAbsAbandon[8] :
                if not os.path.exists(pathImpressions+os.sep+"Course_"+nomFichier+ ".pdf") :
                ...
                if StatsEffectifs :
                    ...
                    if effTotG :
                        ...
                        GArr = "-"
                    if effTotF :
                        FD = str(ArrDispAbsAbandon[2]) 
                    else :
                        FD = "-"
                    if effTotG :
                        GD = str(ArrDispAbsAbandon[3]) 
                    else :
                        GD = "-"
                    if effTotF :
                        FAba = str(ArrDispAbsAbandon[4]) 
                    else :
                        FAba = "-"
                    if effTotG :
                        GAba = str(ArrDispAbsAbandon[5]) 
                    else :
                        GAba = "-"
                    if effTotF :
                        FAbs = str(ArrDispAbsAbandon[6]) 
                    else :
                        FAbs = "-"
                    if effTotG :
                        GAbs = str(ArrDispAbsAbandon[7]) 
                    else :
                        GAbs = '"-"'
                else :
                ...
                ContenuLignesGroupements += [eval(ligneStats[0].replace("@classe",ajouteGuillemets(groupementAPartirDeSonNom(classe).nom)).replace("@FArr",ajouteGuillemets(FArr))\
                             .replace("@GArr",ajouteGuillemets(GArr)).replace("@FD",ajouteGuillemets(FD))\
                             .replace("@GD",ajouteGuillemets(GD)).replace("@FAba",ajouteGuillemets(FAba))\
                             .replace("@GAba",ajouteGuillemets(GAba)).replace("@FAbs",ajouteGuillemets(FAbs))\
                             .replace("@GAbs",ajouteGuillemets(GAbs)).replace("@moy",ajouteGuillemets(moyenne))\
                             .replace("@med",ajouteGuillemets(mediane)))]
    ...
    # fstats += [PageBreak()]
    fstats += ["<p>"]
    fstats += ["<b> Statistiques par groupement :</b>","<p>"]
    # print(ContenuLignesGroupements[0])
    largeurDesColonnesParSexe = 30
...
def absentsDispensesAbandonsNG() :
    Labs = []
    Ldisp = []
    Labandon = []
    #print(Resultats)
    for c in Coureurs.liste() :
        if c.absent :
            ...
            #print(c.nom, c.prenom, "a abandonné")
    print("nbre abs", len(Labs), "  nbre disp",len(Ldisp), "  nbre abandons", len(Labandon))
    intitules = ["ABSENTS", "DISPENSES", "ABANDONS"]
    largeurColonnes = 230
    retour = []
    nbreDeNomsParLigne = 2 
    for n,liste in enumerate([Labs, Ldisp, Labandon]):
        if liste : # la liste est non vide, on affiche les noms correspondants
            retour += ["<center><font size='14'><b><u>" + intitules[n] + "</u></b></font></center>", "<p>", ]
            premiereLigne = [["Noms prénoms",largeurColonnes]]
            for z in range(nbreDeNomsParLigne-1) :
                premiereLigne.append(["",largeurColonnes])
            tableau = [premiereLigne]
            i = 0
            ligne = []
            for c in liste :
                # création d'un tableau de trois colonnes que l'on complète avec chaineCoureur
                chaineCoureur = c.nom + " " + c.prenom + " ("
                if Parametres["CategorieDAge"] == 2 : 
                    chaineCoureur += c.etablissement
                elif Parametres["CategorieDAge"] == 1 : 
                    chaineCoureur += c.categorie(Parametres["CategorieDAge"])
                else :
                    chaineCoureur += c.classe 
                chaineCoureur += ")"
                ligne.append(chaineCoureur)
                # on gère les différents cas.
                if i % nbreDeNomsParLigne == nbreDeNomsParLigne - 1 : # on dispose d'une ligne complète
                    tableau.append(ligne)
                    ligne = []
                i+=1
            if ligne :
                # on complète la dernière ligne si besoin jusqu'à 3
                for z in range(nbreDeNomsParLigne-len(ligne)) :
                    ligne.append("-")
                tableau.append(ligne)
            retour.append(tableau)
            retour.append("<p>")
    return retour
...
##                    nomStandard = addCourse(course)
            ...
            if type(dossard) == Erreur :
                ...
                print("Impossible d'ajouter le coureur ", nom, prenom, sexe, classe, naissance, etablissement, etablissementNature, "dans course",\
                  lettreCourse, " ",course,"avec le dossard", dossard)
            else :
...
def creerCoureur(listePerso, informations) :
    ...
    if "classe" in informations :
        try :
            ...
            clas = supprLF(infos["classe"])
            # si la classe est une chaine de caractère avec des caractères inutiles (comme certains exports siecle), on fait du ménage : ="47" devient 47
            if type(clas) == str :
                clas = clas.replace('"','').replace('=','')
...
def deposePagesHTMLInternet(liste, remplacer=True):
    ...
    try:
        ...
        for file in liste:
            chemin_relatif = os.path.relpath(file, dossier_www)  # Chemin relatif par rapport à "./www"
...
def genereAffichageWWW(listeDesGroupements) :
    ...
    for fichier in os.listdir(dossier_web_resultats) :
        if "Affichage-tab" in fichier :
            os.remove(os.path.join(dossier_web_resultats, fichier))
    ## création des contenus à partir des données de courses.
    onglets = ""
    heuresDeparts = []
    timerID = []
    ...
    for groupement in listeDesGroupements :
        ...
        fichierTabActuel = os.path.join(dossier_web_resultats,"Affichage-tab" + str(i) + ".html")
    ...
    fichierIndex = os.path.join(dossier_web_resultats,"index.html")
